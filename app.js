document.addEventListener('DOMContentLoaded', () => {
    ...
    for (var i = 0, len = squares.length; i < len; i++) {
        ...
        // check board for win/loss
        function checkBoard() {
            // take 4 values from winningArrays and check for squares
            for (let y = 0; y < winningArrays.length; y++) {
                ...
                // check array if it belongs to player-one class
                if (square1.classList.contains('player-one')
                    && square2.classList.contains('player-one')
                    && square3.classList.contains('player-one')
                    && square4.classList.contains('player-one')) {
                    // if true, player-one is winner
                    result.innerHTML = 'Player One Wins!'
                }
                // check array if it belongs to player-two class
    ...
    // eventListener to each square click to trigger checkBoard
